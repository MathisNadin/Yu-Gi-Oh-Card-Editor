...
export function timeZoneList(): string[] {
  ...
  for (let record of RECORDS) {
    result = result.concat(record.utc);
  }
  result.sort();
  return result;
}
...
export function isDaySavingTime(date: Date) {
  ...
  let previousSunday = date.getDate() - date.getDay();
...
export function toUTC(date: Date, timezone: string) {
  return date.clone().addMinutes(-timeZoneOffset(date, timezone));
}

export function toLocale(date: Date, timezone: string) {
  return date.clone().addMinutes(+timeZoneOffset(date, timezone));
}
