...
import { Icon, IIconProps, IIconState } from '../icon';
...
export class PictureEditor extends Containable<IPictureEditorProps, IPictureEditorState> {
  ...
  private async onTap(e: React.MouseEvent<HTMLElement>) {
    ...
    let eventOrRect: React.MouseEvent | DOMRect;
  ...
  public override renderClasses() {
    ...
    classes[`${this.props.display}-display`] = true;
    classes[`${this.props.size}-size`] = true;
    return classes;
  }
  ...
  public override get children(): JSXElementChildren {
    ...
    return [
      <ToolkitImage key='image' src={displayUrl || placeholder} alt='Image' />,
      <HorizontalStack
        key='icon-container'
        ...
        bg='3'
      >
        <Icon key='icon' ref={this.iconRef} icon='toolkit-camera-full' color='1' />
      </HorizontalStack>,
    ];
  }
}
