...
> extends AbstractPopover<P, S> {
  public static get defaultProps(): Partial<IWalkthroughPopoverProps> {
    return {
      ...super.defaultProps,
      bg: 'secondary',
      triangleWidth: 25,
  ...
  protected override calculatePosition() {
    ...
    let preferredHeight: IWalkthroughPopoverState['style']['height'] = syncHeight
  ...
  protected override checkHeight() {
    if (this.state.style.height !== 'fit-content' || this.state.visible || !this.popover?.current?.base?.current) {
      return;
    }
    ...
    const preferredHeight = this.popover.current.base.current.clientHeight;
    const triangleHeight = this.props.triangleHeight! * 2; // To add some margin
  ...
  protected override renderContent(): JSXElementChild {
    return [
